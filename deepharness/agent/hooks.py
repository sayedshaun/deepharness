...
One object with four optional methods rather than a chain of middlewares: a
chain means documenting precedence, and "which one won" is a class of bug that
a single hook does not have. Future steps into the loop arrive here as another
method with a no-op default, so an Agent gains capabilities without gaining a
...
path, not arun() wrapped in an event loop, so an async hook would either need a
second form or silently work under arun() alone. Work that has to await belongs
in a tool, which the loop already dispatches both ways.
"""
...
class Hooks:
    ...
    What each method may change, and what it must not: none of them decide
    ...
    def before_tool(self, call: ToolCall) -> ToolCall | None:
        ...
        rather than a hook being able to slip a call past a deny rule by
        rewriting it afterwards. A refused call never runs and the model is told
        so, exactly as a denial is.
        """
        return call
    ...
    def after_tool(self, call: ToolCall, result: Any) -> Any:
        ...
        values - so a hook can turn a failure into an instruction. What is
